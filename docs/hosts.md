...
`Aerys\Host::use()` is the ubiquitous way to install handlers, `Filter`s, `Bootable`s, and the `HttpDriver`.
...
The execution order of `Filter`s and `Bootable`s solely depends on the order they are passed to `use()` and are always all called. Refer to [the `Filter`s guide](..md).
