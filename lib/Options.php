...
try {
    if (!isset($__aerys_eval_options) && @\assert(false)) {
        goto optimized_options;
    }
...
optimized_options:
    // optimize options access on zend.assertions=-1 runs by allowing direct access
    $__aerys_eval_options = true;
    eval("?>" . str_replace("private $", "public $", file_get_contents(__FILE__)));
    unset($__aerys_eval_options);
}