...
function hpack_huffman_lookup_init() {
    $encodingAccess = [];
    $terminals = [];

    foreach (HPACK_CODE as $chr => $bits) {
        $len = HPACK_CODE_LENGTHS[$chr];
        for ($bit = 0; $bit < 8; $bit++) {
            $offlen = $len + $bit;
...
function hpack_huffman_decode($input) {
    static $hpack_huffman_lookup;

    $lookup = $hpack_huffman_lookup ?? $hpack_huffman_lookup = hpack_huffman_lookup_init();
    $len = \strlen($input);
...
function hpack_huffman_codes_init() {
    $lookup = [];
...
function hpack_huffman_lens_init() {
    $lens = [];

    for ($chr = 0; $chr <= 0xFF; $chr++) {
        $lens[chr($chr)] = HPACK_CODE_LENGTHS[$chr];
    }

    return $lens;
}

function hpack_huffman_encode($input) {
    static $hpack_huffman_codes, $hpack_huffman_lens;
    if ($hpack_huffman_codes == null) {
        $hpack_huffman_codes = hpack_huffman_codes_init();
        $hpack_huffman_lens = hpack_huffman_lens_init();
    }
    ...
    for ($bitcount = $i = 0; $i < $len; $i++) {
        $chr = $input[$i];
        $byte = $bitcount >> 3;
        foreach ($hpack_huffman_codes[$bitcount % 8][$chr] as $bits) {
            $out[$byte] = $out[$byte] | $bits;
            $byte++;
        }
        $bitcount += $hpack_huffman_lens[$chr];
    }
...
const HPACK_LAST_INDEX = 61;
const HPACK_TABLE = [ // starts at 1
    [":authority", ""],
    [":method", "GET"],
    [":method", "POST"],
    [":path", "/"],
...
function hpack_decode_dynamic_integer(&$input) {
    $c = ord($input);
    $int = $c & 0x7f;
    $i = 1;
    while ($c & 0x80) {
...
function hpack_table_resize($table) {
    while ($table->size > $table->maxSize) {
        list($name, $value) = array_pop($table->headers);
        $table->size -= 32 + \strlen($name) + \strlen($value);
    }
}

function hpack_decode($input, $table) {
    $headers = [];
    ...
    while ($input != "") {
        ...
        if ($index & 0x80) {
            // range check
            if ($index <= HPACK_LAST_INDEX + 0x80) {
                $headers[] = HPACK_TABLE[$index - 0x81];
        } else {
                if ($index == 0xff) {
                    $index = hpack_decode_dynamic_integer($input) + 0xff;
                }
                $headers[] = $table->headers[$index - 0x81 - HPACK_LAST_INDEX];
            }
            ...
            if ($index & ($dynamic ? 0x3f : 0x0f)) {
                if ($dynamic ? $index == 0x7f : (($index & 0x1f) == 0x1f)) {
                    $index = hpack_decode_dynamic_integer($input) + ($dynamic ? 0x40 : 0x10);
                } else {
                    $index &= $dynamic ? 0x3f : 0x0f;
                }
                if ($index <= HPACK_LAST_INDEX) {
                    $header = HPACK_TABLE[$index - 1];
                } else {
                    $header = $table->headers[$index - 1 - HPACK_LAST_INDEX];
                }
            } else {
                $len = ord($input);
                $huffman = $len & 0x80;
                ...
                $input = substr($input, 1);
                if ($len == 0x7f) {
                    $len = hpack_decode_dynamic_integer($input) + 0x7f;
                }
                if ($huffman) {
                    $header = [hpack_huffman_decode(substr($input, 0, $len))];
                } else {
            ...
            if ($huffman) {
                $header[1] = hpack_huffman_decode(substr($input, 0, $len));
            } else {
                $header[1] = substr($input, 0, $len);
            }
            $input = substr($input, $len);
            if ($dynamic) {
                array_unshift($table->headers, $header);
                $table->size += 32 + \strlen($header[0]) + \strlen($header[1]);
                if ($table->maxSize < $table->size) {
                    hpack_table_resize($table);
                }
            }
            $headers[] = $header;
        } else { //if ($index & 0x20) {
            if ($index == 0x3f) {
                $input = substr($input, 1);
                $index = hpack_decode_dynamic_integer($input) + 0x40;
            }
...
function hpack_encode_dynamic_integer($int) {
    $out = "";
    $i = 0;
    while ($int >> $i) {
...
function hpack_encode($headers) {
    ...
    foreach ($headers as $name => $value) {
        ...
        foreach (HPACK_TABLE as $index => list($header_name)) {
            if ($name == $header_name) {
                break;
            }
        }
        if ($name == $header_name) {
            ...
            $output[] = "\0\x7f" . hpack_encode_dynamic_integer(\strlen($name) - 0x80) . $name;
        }
        if (\strlen($value) < 0x7f) {
            $output[] = chr(\strlen($value)) . $value;
        } else {
            $output[] = "\x7f" . hpack_encode_dynamic_integer(\strlen($value) - 0x80) . $value;
        }
    }
