...
class WatcherProcess extends Process {
    ...
    private function bindCommandServer(string $config) {
        $path = CommandClient::socketPath(Internal\selectConfigFile($config));
    ...
    protected function recommendLogLevel(Console $console) {
        ...
        if (Logger::LEVELS[$level] === Logger::LEVELS[Logger::DEBUG]) {
            $this->logger->debug(
                "Running aerys in production with a log level equal to \"debug\" is not " .
