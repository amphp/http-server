...
use Amp\Http\Server\HttpServerLifecycle;

final class PerformanceRecommender
{
    public function onStart(HttpServerLifecycle $lifecycle): void
    {
        $logger = $lifecycle->getLogger();

        if ($lifecycle->getOptions()->isInDebugMode()) {
