...
class HostBinder {
     ...
     */
    function bindHosts(HostCollection $hosts, array $existingSockets = []) {
        if (!$hosts->count()) {
     ...
     * @return array Returns an array of bound sockets mapped by address
     */
    function bindAddresses(array $bindAddresses, array $existingSockets = []) {
        if (!$bindAddresses) {
            throw new \LogicException(
     ...
     */
    function setSocketBacklogSize($size) {
