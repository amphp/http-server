...
class Websocket {
    ...
    private function validateClientHandshake(array $asgiEnv) {
        ...
        if (isset($this->endpoints[$requestUri])) {
            ...
            return [FALSE, [Status::NOT_FOUND, Reasons::HTTP_404, [], NULL]];
        }
        ...
        if ($asgiEnv['REQUEST_METHOD'] != 'GET') {
            return [FALSE, [Status::METHOD_NOT_ALLOWED, Reasons::HTTP_405, [], NULL]];
        }
        
        if ($asgiEnv['SERVER_PROTOCOL'] < 1.1) {
            return [FALSE, [Status::HTTP_VERSION_NOT_SUPPORTED, Reasons::HTTP_505, [], NULL]];
        }
        
        if (empty($asgiEnv['HTTP_UPGRADE']) || strcasecmp($asgiEnv['HTTP_UPGRADE'], 'websocket')) {
            return [FALSE, [Status::UPGRADE_REQUIRED, Reasons::HTTP_426, [], NULL]];
        }
        ...
        if ($allowedOrigins && !in_array($originHeader, $allowedOrigins)) {
            return [FALSE, [Status::FORBIDDEN, Reasons::HTTP_403, [], NULL]];
        }
    ...
    private function beforeHandshake(callable $beforeHandshake, array $asgiEnv) {
        try {
            ...
            return [Status::INTERNAL_SERVER_ERROR, Reasons::HTTP_500, [], NULL];
        }
    }
    ...
    private function generateServerHandshake(array $asgiEnv, $version, $subprotocol, $extensions) {
        ...
        return [Status::SWITCHING_PROTOCOLS, Reasons::HTTP_101, $headers, NULL, [$this, 'importSocket']];
    }
